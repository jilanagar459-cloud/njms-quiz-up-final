...
export async function getLatestEndedSession(): Promise<QuizSession | null> {
  const { data } = await supabase
    .from('quiz_sessions')
    .select('*')
    .eq('status', 'ended')
...
// ---- Quiz Presence ----
export async function upsertPresence(sessionId: string, userId: string): Promise<void> {
  await supabase
    .from('quiz_presence')
    .upsert(
      { quiz_session_id: sessionId, user_id: userId, last_seen_at: new Date().toISOString() },
      { onConflict: 'quiz_session_id,user_id' }
    );
}

export async function removePresence(sessionId: string, userId: string): Promise<void> {
  await supabase
    .from('quiz_presence')
    .delete()
    .eq('quiz_session_id', sessionId)
...
export async function getPresenceForSession(sessionId: string): Promise<PresenceEntry[]> {
  const { data } = await supabase
    .from('quiz_presence_details')
    .select('user_id,name,surname,tehsil,phone,joined_at')
    .eq('quiz_session_id', sessionId)
