...
import { getActiveQuizSession, getLeaderboard, getLatestEndedSession } from '@/services/api';
...
export default function LeaderboardPage() {
  ...
  const loadLeaderboard = useCallback(async () => {
    // Try active session first; fall back to most recent ended session
    let s = await getActiveQuizSession();
    if (!s) s = await getLatestEndedSession();
    setSession(s);
    if (s) {
